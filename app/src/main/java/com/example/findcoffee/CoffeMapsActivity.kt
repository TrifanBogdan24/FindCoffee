...
class CoffeeMapsActivity : ComponentActivity() {
    ...
    private val requestPermissionLauncher = registerForActivityResult(
        ...
        if (isGranted) {
            // Daca utilizatorul a acceptat, reincarca activitatea pentru a activa harta
            recreate()
        } else {
                ...
                AndroidView(
                    ...
                    factory = { ctx ->
                        MapView(ctx).apply {
                            ...
                            getMapAsync { map ->
                                googleMap = map
                                // Harta va fi configurata dupa ce are permisiuni
        ...
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)
            ...
            fusedLocationClient.lastLocation.addOnSuccessListener { location ->
                if (location != null) {
                    ...
                    // Pentru a vedea (cafenelele) la nivel de strada -> zoom=15f
                    map.animateCamera(CameraUpdateFactory.newLatLngZoom(userLatLng, 15f))

                    // Calculeaza cafenelele in jurul punctului gasit
    ...
    private fun searchNearbyCoffees(location: LatLng) {
        ...
        // Parametri de query catre Google (Maps)
